	      ...
	      TCL_Char **argv)
{
  ...
  //    int numEigen;
    ...
    */
    ...
    if (result < 0) {
      opserr << "eigenAnalysis - eigenSOE failed\n";
      return TCL_ERROR;
    }
    
    //
    // get the eigenvalues and set them in the domain
    //
...
#endif
#ifdef _PARALLEL_INTERPRETERS
   // If interpreters are parallel but not processing distributed (e.g. parametric), 
   // we might not want to reduce, or maybe we do. 
   // Standard OpenSeesMP defines _PARALLEL_PROCESSING.
   // OpenSeesSP defines _PARALLEL_INTERPRETERS.
   // Assuming OpenSeesMP context for domain decomposition.
   // If standard OpenSeesMP, _PARALLEL_PROCESSING is defined.
#endif
